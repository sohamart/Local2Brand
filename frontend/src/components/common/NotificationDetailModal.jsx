...
import { X, Mail, ExternalLink, Clock, Shield, Tag, CheckCircle2, User, Loader2 } from 'lucide-react';
import { Link } from 'react-router-dom';
import notificationApi from '../../services/notificationApi';

export default function NotificationDetailModal({ notification, onClose, onMarkRead }) {
  const [detail, setDetail] = useState(notification || null);
  const [loadingDetail, setLoadingDetail] = useState(false);

  useEffect(() => {
    setDetail(notification);
    if (notification?._id && !notification?.emailHtml) {
      setLoadingDetail(true);
      notificationApi.getNotificationById(notification._id)
        .then((res) => {
        ...
        .catch((err) => {
          console.warn('Error fetching full email replica:', err);
        })
        .finally(() => {
    ...
    <div className="fixed inset-0 z-[999999999] flex items-center justify-center p-2.5 sm:p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200 overflow-y-auto">
      ...
      >
          ...
          <div className="flex items-start gap-3 pr-2 sm:pr-4">
            ...
            <div className="space-y-1">
              <div className="flex flex-wrap items-center gap-2">
                <span className={`text-[11px] font-bold px-2.5 py-0.5 rounded-full border ${getCategoryColor(detail.category)}`}>
                  {detail.category || 'Notification'}
                </span>
                {detail.recipientRole === 'admin' && (
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 px-4 sm:px-6 py-2.5 bg-slate-100/50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800/80 text-xs text-slate-500 dark:text-slate-400 font-medium">
          ...
          {detail.recipientEmail && (
            <div className="flex items-center gap-1.5 text-slate-600 dark:text-slate-300">
              ...
              <span>Target: <strong className="font-semibold">{detail.recipientEmail}</strong></span>
            </div>
          )}
        </div>
        ...
        <div className="p-3 sm:p-6 overflow-y-auto flex-1 space-y-3 sm:space-y-4 min-h-0">
          ...
          {/* Render Full Rich HTML Email Replica if present or loading */}
          {loadingDetail ? (
            <div className="py-6 flex items-center justify-center gap-2 text-slate-400 text-xs">
              <Loader2 className="w-4 h-4 animate-spin text-purple-500" />
              <span>Loading complete email replica...</span>
            </div>
          ) : detail.emailHtml ? (
            <div className="space-y-2 pt-1">
              ...
              <div 
                ...
                dangerouslySetInnerHTML={{ __html: detail.emailHtml }}
              />
            </div>
          ) : null}
        </div>
        ...
        <div className="flex items-center justify-between gap-2.5 p-3 sm:p-5 border-t border-slate-100 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-900/30 shrink-0">
          <div className="text-xs text-slate-400">
            {detail.isRead ? (
              ...
              <button
                type="button"
                onClick={() => onMarkRead && onMarkRead(detail._id)}
