...
export default function SmartRequirementModal() {
  ...
  const { isInquiryOpen, closeOrderModal, openCallbackModal, inquiryData } = useOrderModal();
  ...
  const normalizeCategoryKey = (catOrSlug) => {
    if (!catOrSlug) return 'restaurant';
    const norm = String(catOrSlug).toLowerCase().trim().replace(/[- ]/g, '_');
    if (norm.includes('lms') || norm.includes('course') || norm.includes('skillcraft') || norm.includes('coaching') || norm.includes('edtech') || norm.includes('education')) return 'coaching';
    if (norm.includes('restaurant') || norm.includes('dining') || norm.includes('food') || norm.includes('spice')) return 'restaurant';
    if (norm.includes('cafe') || norm.includes('bakery') || norm.includes('coffee') || norm.includes('brew')) return 'cafe';
    if (norm.includes('salon') || norm.includes('spa') || norm.includes('beauty') || norm.includes('glamour') || norm.includes('barber')) return 'salon';
    if (norm.includes('gym') || norm.includes('fitness') || norm.includes('crossfit') || norm.includes('workout') || norm.includes('iron')) return 'gym';
    if (norm.includes('hotel') || norm.includes('resort') || norm.includes('homestay') || norm.includes('villa') || norm.includes('stay')) return 'hotel';
    if (norm.includes('real_estate') || norm.includes('property') || norm.includes('builder') || norm.includes('estate') || norm.includes('realty')) return 'real_estate';
    if (norm.includes('photo') || norm.includes('studio') || norm.includes('wedding') || norm.includes('lens')) return 'photography';
    if (norm.includes('boutique') || norm.includes('cloth') || norm.includes('fashion') || norm.includes('ecommerce') || norm.includes('e_commerce') || norm.includes('apparel')) return 'boutique';
    if (norm.includes('clinic') || norm.includes('doctor') || norm.includes('health') || norm.includes('dental') || norm.includes('care')) return 'clinic';
    if (norm.includes('jewel') || norm.includes('gold') || norm.includes('diamond') || norm.includes('silver')) return 'jewellery';
    if (norm.includes('auto') || norm.includes('car') || norm.includes('bike') || norm.includes('showroom') || norm.includes('motor') || norm.includes('ev')) return 'showroom';
    return 'custom';
  };

  const currentStep = STEPS[currentStepIndex] || STEPS[0];
  const matchedDbDemo = dbDemos.find((d) => d.slug === formData.websiteType || d._id === formData.websiteType);
  const currentCategoryKey = normalizeCategoryKey(formData.websiteType || matchedDbDemo?.category || inquiryData?.category);
  const activeCategorySpec =
    CATEGORY_SPECS[currentCategoryKey] ||
  ...
  // Find dynamic custom questions from published form schema configured in Admin Form Builder
  const dynamicCategoryQuestions = (formSchema?.questions || []).filter(
    (q) => q.stepId === 'step_business' && (q.categoryId === currentCategoryKey || q.categoryId === formData.websiteType || q.categoryId === 'all')
  );
  ...
  return (
    <div className="fixed inset-0 z-[9999999] flex items-center justify-center p-2 sm:p-4 overflow-y-auto bg-slate-950/85 backdrop-blur-2xl animate-in fade-in duration-200">
      ...
      <div className="relative w-full max-w-4xl bg-white dark:bg-slate-900 rounded-3xl shadow-[0_0_60px_rgba(168,85,247,0.25)] border-2 border-purple-500/40 dark:border-purple-500/30 overflow-hidden my-auto max-h-[94vh] flex flex-col">
        ...
        {/* APPLIED TEMPLATE PROMO BANNER */}
        {inquiryData && (inquiryData.selectedDemo || inquiryData.templateId) && (
          <div className="bg-gradient-to-r from-purple-950 via-slate-900 to-purple-950 border-b border-purple-500/30 px-3.5 sm:px-5 py-2 flex items-center justify-between gap-3 text-xs shrink-0">
            <div className="flex items-center gap-2 sm:gap-2.5 min-w-0">
              <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-lg sm:rounded-xl bg-gradient-to-tr from-purple-600 to-pink-600 text-white flex items-center justify-center font-bold shrink-0 shadow-xs">
                <Sparkles className="w-3.5 h-3.5" />
              </div>
              <div className="min-w-0">
                <span className="text-[9px] sm:text-[10px] uppercase font-black text-purple-300 tracking-wider block leading-tight">
                  Selected Template Auto-Configured
                </span>
                <span className="font-bold text-white truncate text-xs sm:text-sm block">
                  {formData.websiteTypeName || inquiryData.selectedDemo}
                </span>
              </div>
            </div>
            <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
              <span className="font-black text-emerald-400 text-xs sm:text-sm">
                {formData.budget || '₹4,999'}
              </span>
              <span className="px-2 py-0.5 rounded-full bg-amber-400/20 border border-amber-400/40 text-amber-300 font-black text-[9px] sm:text-[10px] uppercase tracking-wide">
                🎁 20% OFF
              </span>
              <span className="px-2 py-0.5 rounded-full bg-purple-500/20 border border-purple-500/40 text-purple-300 font-semibold text-[9px] sm:text-[10px] hidden sm:inline">
                ⚡ 48h Delivery
              </span>
            </div>
          </div>
              ...
              <div className="pt-2 flex flex-wrap items-center justify-center gap-3">
                ...
                <button
                  ...
                  onClick={() => {
                    closeOrderModal();
                    openCallbackModal({
                      topic: `Priority Discussion for Order ${submittedData.requirementId}`,
                      source: 'Requirement Form Submitted',
                      name: submittedData.clientInfo?.ownerName || '',
                      phone: submittedData.clientInfo?.mobile || '',
            ...
            <div className="space-y-6">
              ...
              {currentStepIndex === 2 && (
                <div className="space-y-5 animate-in fade-in">
                  ...
                  {/* Dynamic Fields List (from active category specs and admin form builder) */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                    ...
                    {/* Dynamic Questions Configured Live in Admin Form Builder */}
                    {dynamicCategoryQuestions
                      ?.filter((q) => !activeCategorySpec.fields?.some((f) => f.id === q.id || f.label === q.title))
                      .map((q) => (
                        <div
                          key={q.id}
                          className={`p-4 rounded-2xl bg-purple-50/30 dark:bg-purple-950/20 border border-purple-200 dark:border-purple-800/60 space-y-2 ${
                            ['textarea'].includes(q.type) ? 'sm:col-span-2' : ''
                          }`}
                        >
                          <label className="text-xs font-bold text-slate-800 dark:text-slate-200 block">
                          ...
                          {['select', 'radio'].includes(q.type) && (
                            <select
                              value={formData.businessDetails[q.id] || ''}
                              onChange={(e) => setFormData((prev) => ({
                                ...prev,
                                businessDetails: { ...prev.businessDetails, [q.id]: e.target.value }
                              }))}
                              className="w-full p-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-900 dark:text-white"
                            >
                              <option value="">Select option...</option>
                          ...
                          {['multiselect', 'checkbox'].includes(q.type) && (
                            <div className="space-y-1.5">
                              {(q.options || []).map((opt) => {
                                const checked = (formData.businessDetails[q.id] || []).includes(opt);
                                return (
                                  <label key={opt} className="flex items-center gap-2 text-xs font-semibold cursor-pointer">
                                    <input
                                      type="checkbox"
                                      checked={checked}
                                      onChange={() => {
                                        const cur = formData.businessDetails[q.id] || [];
                                        const upd = checked ? cur.filter((x) => x !== opt) : [...cur, opt];
                                        setFormData((prev) => ({
                                          ...prev,
                                          businessDetails: { ...prev.businessDetails, [q.id]: upd }
                                        }));
                                      }}
                                    />
                                    <span>{opt}</span>
                          ...
                          {['text', 'phone', 'email', 'number'].includes(q.type) && (
                            <input
                    ...
                    {/* Dedicated Custom User Input & Special Requirements Field */}
                    <div className="sm:col-span-2 p-4 rounded-2xl bg-gradient-to-r from-purple-500/10 via-pink-500/10 to-purple-500/10 border-2 border-purple-300 dark:border-purple-700/80 space-y-2">
                      <div className="flex items-center gap-2">
                        <Edit3 className="w-4 h-4 text-purple-600 dark:text-purple-400 shrink-0" />
                        <label className="text-xs font-bold text-slate-900 dark:text-white">
                          Custom Requirements, Unique Features or Specific Notes (Optional):
                        </label>
                      </div>
                      <textarea
                        rows={2}
                        value={formData.businessDetails.customNotes || formData.additionalNotes || ''}
                        onChange={(e) => {
                          const val = e.target.value;
                          setFormData((prev) => ({
                            ...prev,
                            additionalNotes: val,
                            businessDetails: { ...prev.businessDetails, customNotes: val }
                          }));
                        }}
                        placeholder="e.g. We need automated SMS OTP verification for orders, a dark luxury color scheme, and direct delivery pincode checking."
                        className="w-full p-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-xs text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-purple-500 leading-relaxed resize-none"
                      />
                    </div>
                  </div>
