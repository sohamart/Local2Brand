...
export default function MaintenanceMode({ onBypassSuccess }) {
  // Target Grand Launch Date: 3 Days from now
  ...
  const handleInstaSubmit = (e) => {
    ...
    const textToCopy = `✨ LOCAL2BRAND EARLY ACCESS INQUIRY ✨\n` +
      `👤 Name: ${formData.name}\n` +
      `🏢 Business: ${formData.businessName}\n` +
      (formData.instagramHandle ? `📸 My IG: ${formData.instagramHandle}\n` : '') +
      (formData.email ? `📧 Email: ${formData.email}\n` : '') +
      `📝 Requirements: ${formData.requirements || 'Website inquiry'}\n` +
  ...
  const handleAdminLogin = (e) => {
    e.preventDefault();
    const correctPassword = import.meta.env.VITE_ADMIN_BYPASS_KEY || 'l2b2025admin';
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#07090E] flex flex-col items-center justify-between p-4 sm:p-8 relative overflow-hidden font-sans text-slate-900 dark:text-slate-100 selection:bg-pink-600 selection:text-white transition-colors duration-300">

      {/* Background Ambient Liquid Glows */}
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[600px] h-[350px] bg-gradient-to-tr from-pink-400/20 via-purple-500/20 to-indigo-500/25 dark:from-pink-500/15 dark:via-purple-600/20 dark:to-indigo-600/25 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute -bottom-20 -right-20 w-[400px] h-[400px] bg-amber-400/15 dark:bg-amber-500/10 rounded-full blur-[100px] pointer-events-none" />

      {/* Top Header Branding */}
      <header className="w-full max-w-6xl mx-auto flex items-center justify-between py-4 relative z-10">
        <div className="flex items-center gap-2.5">
          <div className="w-9 h-9 rounded-xl overflow-hidden shadow-md shadow-purple-500/20 border border-white/80 dark:border-slate-700 bg-white dark:bg-slate-900">
            <img
              src="/logo.jpg"
        ...
        <a
          ...
          className="px-3.5 py-1.5 rounded-full text-xs font-bold text-pink-800 dark:text-pink-300 bg-pink-50 dark:bg-pink-950/70 hover:bg-pink-100 dark:hover:bg-pink-900/80 border border-pink-200/80 dark:border-pink-500/40 transition-all flex items-center gap-1.5 shadow-2xs cursor-pointer"
        >
      ...
      <main className="w-full max-w-4xl mx-auto text-center space-y-8 py-6 sm:py-10 relative z-10">
        ...
        {/* Status Pill Badge */}
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/95 dark:bg-slate-900/90 backdrop-blur-xl border border-slate-200/80 dark:border-slate-700/80 shadow-sm text-xs font-bold text-slate-800 dark:text-slate-200 animate-float">
          <span className="w-2 h-2 rounded-full bg-pink-500 animate-ping" />
          <span className="text-pink-900 dark:text-pink-300">
            {siteConfig.isMaintenanceMode ? '🛠️ Scheduled Infrastructure Upgrade' : '🚀 Grand Platform Launch'}
          </span>
          <span className="text-slate-300 dark:text-slate-700">|</span>
          <span className="text-purple-600 dark:text-purple-400">48h Delivery Coming Soon</span>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-6 max-w-xl mx-auto">
          {/* Days */}
          <div className="glass-panel p-3.5 sm:p-5 rounded-2xl border border-white dark:border-slate-700/80 shadow-floating flex flex-col items-center justify-center relative overflow-hidden">
          ...
          {/* Seconds */}
          <div className="glass-panel p-3.5 sm:p-5 rounded-2xl border border-white dark:border-slate-700/80 shadow-floating flex flex-col items-center justify-center relative overflow-hidden">
            <span className="text-3xl sm:text-4xl font-black text-pink-600 dark:text-pink-400 tracking-tight font-mono animate-pulse">
        ...
        <div className="max-w-xl mx-auto text-left pt-2">
          <div className="glass-panel p-5 sm:p-7 rounded-3xl border border-white dark:border-slate-700/80 shadow-xl relative overflow-hidden bg-white/95 dark:bg-slate-900/95">
            <div className="mb-4 text-center">
              <span className="text-xs font-bold uppercase tracking-widest text-pink-700 dark:text-pink-400 inline-flex items-center gap-1.5 mb-1">
            ...
            <form onSubmit={handleInstaSubmit} className="space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 text-xs text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-pink-500"
                  />
                </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleFormChange}
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 text-xs text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-pink-500 resize-none"
                ></textarea>
              </div>
      ...
      <footer className="w-full max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 pb-2 border-t border-slate-200/80 dark:border-slate-800 text-xs text-slate-500 dark:text-slate-400 relative z-10">
        <div className="flex items-center gap-1.5">
          <AshokaChakra size={11} />
      ...
      {showInstaModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-2xl border border-white dark:border-slate-700 z-10 animate-in fade-in zoom-in-95 text-center space-y-3">
      ...
      {showAdminModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div
            role="dialog"
            aria-modal="true"
            className="relative w-full max-w-sm bg-white/98 dark:bg-slate-900/98 backdrop-blur-2xl rounded-3xl border border-white dark:border-slate-700 p-6 shadow-2xl z-10 animate-in fade-in zoom-in-95 space-y-4"
          >
