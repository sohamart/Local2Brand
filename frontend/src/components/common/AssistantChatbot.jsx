...
export default function AssistantChatbot() {
  ...
  return (
    <div className="fixed bottom-4 sm:bottom-6 right-4 sm:right-6 z-[99999]">
      ...
      {/* Glassmorphic Drawer Window with Animated Neon Border */}
      {isOpen && (
        <div
          data-lenis-prevent="true"
          onWheel={(e) => e.stopPropagation()}
          className="absolute bottom-16 sm:bottom-18 right-0 w-[92vw] sm:w-[420px] h-[600px] max-h-[85vh] p-[2px] rounded-3xl overflow-hidden shadow-2xl shadow-emerald-500/20 dark:shadow-emerald-500/30 animate-in slide-in-from-bottom-5 duration-200 z-[99999] pointer-events-auto relative group"
        >
          {/* Animated Conic Border on Main Drawer */}
          <span className="absolute -inset-[150%] rounded-3xl bg-[conic-gradient(from_0deg,transparent_0_260deg,#059669_290deg,#10b981_330deg,#34d399_360deg)] animate-spin-conic pointer-events-none" />

          {/* Inner Drawer Core */}
          <div className="w-full h-full bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl rounded-[22px] overflow-hidden flex flex-col relative z-10 border border-white/60 dark:border-slate-800/80">
            {/* Header */}
