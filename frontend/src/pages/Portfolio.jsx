        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10 sm:mt-14">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 lg:gap-10">
            {filteredProjects.map((project) => (
              <div
                key={project.id}
                className="glass-panel rounded-card sm:rounded-hero p-5 sm:p-8 border border-white shadow-glass hover:shadow-glass-lg transition-all duration-300 flex flex-col justify-between group relative overflow-hidden"
              >
                <div>
                  {/* Visual Preview */}
                  <div className="relative rounded-xl sm:rounded-2xl overflow-hidden aspect-[16/10] bg-slate-100 mb-6 group border border-white">
                    <img
                      src={project.image}
                      alt={project.title}
                      ...
                      loading="lazy"
                    />
                    <div className="absolute top-3 left-3 px-3 py-1 rounded-full bg-white/90 backdrop-blur-md text-xs font-bold text-slate-900 shadow-sm">
                      {project.category}
                    </div>
                  </div>
                  ...
                  <p className="text-xs sm:text-sm font-semibold text-purple-600 flex items-center gap-1.5 mb-3">
                  ...
                  {/* Metrics Grid */}
                  <div className="grid grid-cols-3 gap-2 sm:gap-3 py-3 border-y border-slate-100 mb-6 text-center sm:text-left">
                    {project.metrics.map((m, i) => (
                      <div key={i}>
                        <div className="text-sm sm:text-base font-extrabold text-slate-900">{m.value}</div>
                ...
                {/* Card Actions */}
                <div className="pt-2 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100/80">
                  <button
