    ...
    <div className="group rounded-card glass-card overflow-hidden border border-white/90 shadow-glass hover:shadow-glass-highlight transition-all duration-300 flex flex-col justify-between relative">
      ...
      <div className="relative aspect-[16/10] overflow-hidden bg-slate-100">
        ...
        <div className="absolute top-3 left-3 right-3 flex items-center justify-between pointer-events-none">
          <span className="px-2.5 sm:px-3 py-1 rounded-full bg-white/95 backdrop-blur-md border border-white text-slate-800 text-[10px] sm:text-[11px] font-bold shadow-sm">
            {demo.category}
          </span>
          {demo.badge && (
        ...
        <div className="hidden sm:flex absolute inset-0 bg-slate-900/35 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-opacity duration-300 items-center justify-center gap-2.5 p-4">
          <Link
            to={`/demos/${demo.slug}`}
            className="px-4 py-2 rounded-btn bg-white text-slate-900 font-bold text-xs shadow-lg hover:bg-slate-50 transition-all flex items-center gap-1.5"
          >
      ...
      <div className="p-5 sm:p-6 flex-1 flex flex-col justify-between space-y-4">
        <div>
          ...
          <div className="flex items-center justify-between text-xs text-slate-500 mb-2">
            <div className="flex items-center gap-1 text-amber-500 font-semibold text-[11px] sm:text-xs">
              ...
              <span className="text-slate-400">({demo.reviewsCount})</span>
            </div>
          ...
          {/* Title */}
          <h3 className="text-lg sm:text-xl font-bold text-slate-900 tracking-tight group-hover:text-purple-600 transition-colors">
          ...
          {/* Short Description */}
          <p className="text-slate-600 text-xs sm:text-sm line-clamp-2 mt-1.5">
        ...
        <div className="pt-3 border-t border-slate-100 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5">
          ...
          <div className="flex items-center gap-2">
            <Link
              to={`/demos/${demo.slug}`}
              className="flex-1 sm:flex-none px-3.5 py-2 rounded-xl text-xs font-bold text-slate-800 bg-slate-100 hover:bg-slate-200 text-center"
            >
              Details
            </Link>
