          ...
          {user ? (
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              {(user?.role === 'delivery' || isAdmin) && (
                <button
                  onClick={onOpenRider}
                  className="px-2 sm:px-2.5 py-1.5 sm:py-2 rounded-lg bg-[#D8632C]/20 hover:bg-[#D8632C]/30 text-[#D8632C] border border-[#D8632C]/40 text-xs font-mono font-bold flex items-center gap-1 transition-colors shrink-0"
                  title="Rider Delivery Hub"
                >
                  <Bike className="w-3.5 h-3.5" />
                  <span className="hidden md:inline">Rider Hub</span>
                </button>
              )}
            </div>
          ) : (
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-x-0 top-[calc(100%)] max-h-[calc(100vh-100px)] overflow-y-auto bg-[#171310]/98 backdrop-blur-xl border-b border-[#A9865A]/30 px-4 pt-4 pb-8 space-y-4 font-mono text-xs animate-fade-in shadow-2xl z-50">
          ...
          <div className="space-y-2 pt-1">
            ...
            {/* Profile / Admin / Rider actions in drawer */}
            {user ? (
              <div className="space-y-2 pt-1">
                {(user.role === 'delivery' || isAdmin) && (
                  <button
                    onClick={() => { setMobileMenuOpen(false); onOpenRider(); }}
                    className="w-full py-3 rounded-xl bg-[#D8632C]/20 border border-[#D8632C]/40 text-[#D8632C] font-bold flex items-center justify-center gap-2 text-xs"
                  >
                    <Bike className="w-4 h-4" />
                    <span>Open Rider Delivery Hub</span>
                  </button>
                )}
